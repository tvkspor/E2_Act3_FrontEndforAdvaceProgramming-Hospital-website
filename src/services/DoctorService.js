...
export const createDoctor = async (data) => {
    const res = await axios.post(`${process.env.REACT_APP_API_URL}/doctor/create`, data)
    return res.data
}

export const getDetailsDoctor = async (id) => {
    const res = await axios.get(`${process.env.REACT_APP_API_URL}/doctor/get-details/${id}`)
    return res.data
}

export const updateDoctor = async (id, access_token, data) => {
    const res = await axiosJWT.put(`${process.env.REACT_APP_API_URL}/doctor/update/${id}`, data, {
        headers: {
...
export const deleteDoctor = async (id, access_token) => {
    const res = await axiosJWT.delete(`${process.env.REACT_APP_API_URL}/doctor/delete/${id}`, {
        headers: {
            token: `Bearer ${access_token}`,
        }
    })
...
export const deleteManyDoctor = async (data, access_token,) => {
    const res = await axiosJWT.post(`${process.env.REACT_APP_API_URL}/doctor/delete-many`, data, {
        headers: {
            token: `Bearer ${access_token}`,
        }
    })
